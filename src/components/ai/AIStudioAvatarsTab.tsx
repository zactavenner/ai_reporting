            ...
            <div className="space-y-1.5">
              ...
              {referenceUrl ? (
                <div className="flex items-center gap-3 rounded-lg border border-border/60 p-2">
                  <img src={referenceUrl} alt="Reference" className="h-16 w-12 rounded-md object-cover" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium">Reference attached</p>
                    <p className="text-[11px] text-muted-foreground">Identity & look will be preserved from this image.</p>
                  </div>
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setReferenceUrl(null)} disabled={generating}>
                    <X className="h-3.5 w-3.5" />
                  </Button>
                </div>
              ) : (
                <label className="flex items-center justify-center gap-2 rounded-lg border border-dashed border-border/70 py-4 text-xs text-muted-foreground cursor-pointer hover:border-primary/50 hover:text-foreground transition">
                  {uploadingRef ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
                  {uploadingRef ? "Uploading…" : "Upload a photo to guide the avatar (face, outfit, vibe)"}
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
                    disabled={uploadingRef || generating}
                    onChange={(e) => { const f = e.target.files?.[0]; if (f) handleReferenceUpload(f); e.currentTarget.value = ""; }}
                  />
                </label>
              )}
              {referenceUrl && model !== "nano-banana-pro" && (
                <p className="text-[11px] text-amber-500">Reference images work best with Nano Banana Pro.</p>
              )}
            </div>
