...
OPERATING RULES:
 ...
 5b. HARD PRODUCTION BUDGETS — these are enforced by the tools against the database, and calling past them is a failure, not initiative:
     ...
     • ${ONBOARDING_VIDEO_BUDGET} videos per client, TOTAL — 30 seconds each, one per natural-motion style: podcast, street_interview, walk_and_talk, broll, split_screen.
...
const TOOLS = [
  ...
  { type: "function", function: { name: "generate_video", description: `Start a real 30-second natural-motion video ad. HARD CAP: ${ONBOARDING_VIDEO_BUDGET} videos per client for the whole onboarding, one per style (podcast, street_interview, walk_and_talk, broll, split_screen). Returns job_id to poll with check_video_job. If the tool reports the budget is exhausted, the videos are finished — never call it again.`, parameters: { type: "object", properties: { client_id: { type: "string" }, creative_id: { type: "string" }, image_url: { type: "string" }, prompt: { type: "string", description: "The approved script/hook for this video. Motion + style direction is added automatically." }, style: { type: "string", description: "podcast | street_interview | walk_and_talk | broll | split_screen" }, duration: { type: "number", description: "Seconds. Defaults to 30." }, aspect_ratio: { type: "string" }, resolution: { type: "string" }, model: { type: "string" } }, required: ["image_url", "prompt"] } } },
