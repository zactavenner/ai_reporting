...
- Every assistant message in the Video Ads agent carries an inline **Generate video with this script** button plus quick 9:16/16:9 and seconds pills; it calls `send(..., { forceProduce: true, videoApproved: true })` so a chat-intent thread can render that one turn without flipping the composer toggle. The render prompt is built from the user's own script — no boilerplate; with a first frame/reference the prompt describes motion, camera and audio only.
